...
SdssLrg10::SdssLrg10(const baseCosmos& c, std::string dataDir)
                     : mCosmos(c), mDataDir(dataDir), mMaxBand(12)
{
  std::string filename = dataDir+"lrgdr7_kbands.txt";
  ifstream kbandFile(filename.c_str());
  if (!kbandFile) throw Bad_Error("SdssLrg10::SdssLrg10() Could not open sdss_lrg data file: " + filename);

  std::copy(std::istream_iterator<double>(kbandFile), std::istream_iterator<double>(),
                                                            std::back_inserter(mkvals));

  filename = dataDir+"lrgdr7_ccmeasurements.txt";
  ifstream measurements(filename.c_str());
  if (!measurements) throw Bad_Error("Could not open sdss_lrg10 data file: " + filename);
  ...
  filename = dataDir+"lrgdr7_windows.txt";
  ifstream windows(filename.c_str());
  if (!windows) throw Bad_Error("Could not open sdss_lrg10 data file: " + filename);
...
std::vector<double> SdssLrg10::shiftedPower(Spline& pk, double bias)
{
...
double SdssLrg10::chi2(Spline& pk, double bias)
{
cout << "SdssLrg10::chi2" << endl; 
...
double SdssLrg10::chi2bestBias(Spline& pk, double firstBias, double lastBias, unsigned int steps)
{
...
double SdssLrg10::chi2margedOverb(Spline& pk)
{
...
double SdssLrg10::chi2margedOverb2Q(Spline& pk, double Ag)
{
    using namespace std;
